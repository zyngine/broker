...
async function purgeAll(guildId) {
  // Nuke every property and every history entry for the guild
  const { rows } = await pool.query(
    `DELETE FROM properties WHERE guild_id = $1 RETURNING *`,
    [guildId]
  );
  await pool.query(
    `DELETE FROM property_history WHERE guild_id = $1`,
    [guildId]
  );
  return rows.length;
}
...
async function getRepossessedProperties(guildId) {
  const { rows } = await pool.query(
    `SELECT * FROM properties WHERE guild_id = $1 AND status = 'repossessed' ORDER BY CASE WHEN property_id ~ '^\\d+' THEN REGEXP_REPLACE(property_id, '[^0-9].*$', '')::BIGINT ELSE NULL END ASC NULLS LAST, property_id ASC`,
    [guildId]
  );
  return rows;
}
...
async function getAllProperties(guildId, { limit, offset }) {
  const { rows } = await pool.query(
    `SELECT * FROM properties WHERE guild_id = $1 ORDER BY CASE WHEN property_id ~ '^\\d+' THEN REGEXP_REPLACE(property_id, '[^0-9].*$', '')::BIGINT ELSE NULL END ASC NULLS LAST, property_id ASC LIMIT $2 OFFSET $3`,
...
async function getAllPropertiesForWeb({ search, status, guildId } = {}) {
  ...
  query += ` ORDER BY CASE WHEN property_id ~ '^\\d+' THEN REGEXP_REPLACE(property_id, '[^0-9].*$', '')::BIGINT ELSE NULL END ASC NULLS LAST, property_id ASC`;
...
module.exports = {
  ...
  createProperty, getProperty, updateProperty, updateNotes, repoProperty, deleteProperty, purgeProperty, purgeAll,
